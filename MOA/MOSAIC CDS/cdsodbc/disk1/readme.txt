...
The CDS ODBC driver is for use with the Microsoft Windows NT and Windows 95 operating systems 
ONLY.  This package contains the Central Data Server ODBC client software.  It must be used in 
conjunction with separately released server software piece known as “Mosaic OA Central Data Server”.
...
? Easier to apply patches to the distribution, if needed.
 V1.4.9
? Setup/.Install now works on Windows 95.   The previous version erroneously installed msvcrt40.dll 
which wreaked havoc in Windows 95 systems.
? Setup now normally doesn't install the data dictionary tools that are usually installed with the mosaic 
Runtime package.   You can install these tools from with this package with the Custom setup option.
? Bug fixes.
 V1.4.8
? Install now works with ODBC version 3.  This version also includes the ODBC 3.0 bug fixes supplied 
by Microsoft on April 25, 1997. 
...
? The ALTER verb is now supported in SQL.   You may add columns or drop columns with an ALTER 
TABLE statement.   But you can’t do both add column and drop column within a single statement.
...
SQLSetConnectOption().
? Return proper SQLState when given a bad table name on an ODBC INSERT 
statement.
? Return proper SQLState when given a bad primary key for an insert or a bad 
secondary key for any update.
...
? CDS ODBC SQL parser allows select FOR UPDATE (with no updatable column list) as well as FOR 
...
? SQLBrowseConnect()
? SQLPutData()
? Support for 16 bit applications.   Olivetti doesn’t plan to support 16 bit CDS applications.
...
? One of: Microsoft Windows 95;  Windows NT 3.51; Windows NT Server 3.51; Windows NT 
...
? Uninstall this CDS ODBC driver package if you already have it 
installed and you aren’t installing on Windows NT 3.51.  This will 
maintain the correct usage counts in the registry, for files that are 
shared with other mosaic OA packages.
? If you are running on Window NT 3.51, you may wish to install WordView at this point, to enable you 
to read the CDS documentation, which is in Microsoft Word 6.0/95 format.   The WordView package 
is included on the CD-ROM with the CDS package.  It is not installed automatically.  To install 
WordView: cd \wordview\disk1 and run setup.exe.
? Install mosaic OA Central Data Server ODBC driver:
? From CD_ROM:
? Run setup.BAT in the root directory of the CD-ROM.  On NT4 and 
Win95, “Control Panel\Add/Remove Programs” automatically 
finds this for you. Reboot the machine to ensure that new 
environment will be used.
 
? From Diskettes:
? Run setup.exe on diskette labeled disk1.  Put in the other diskettes when asked. “Control 
Panel\Add/Remove Programs” automatically finds this for you.
...
time after the initial install.    On Windows 95 this ODBC administrator program is accessed via ‘My 
Computer’ / ‘Control Panel’ / ’32 bit ODBC’.

 How to Uninstall mosaic OA ODBC Driver
 
 On Windows NT 4.0 or Windows 95, use the Settings/Control Panel/”Add/Remove Programs” applet.  
Choose “MOA CDS ODBC” and click on ADD/Remove.   On Windows NT 3.51 double click on the 
Uninstall Icon in the “mosaic OA” program group.
...
? Grant only read permission on your dictionary files to all users or groups of NT users to whom you 
want to grant access to your database.
...
9. Use the right mouse button to click on the Internet icon on your desktop, and then click 
Properties on the menu that appears.
10. Click the Use AutoDial or "Connect to the Internet as needed" check box to clear it.
11. Click OK.
CHANGE LOG

Mosaic OA Central Data Server ODBC Driver "Change & Enhancement" Log

Listed below are all changes implemented in version 4.9.0 
...
18
08/13/97		CDS ODBC driver release notes
08/13/97	1
